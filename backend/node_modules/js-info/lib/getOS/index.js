...
function getOS() {
  if (_.runtime.node) {
    if (process.platform && process.platform.length) {
      if (process.platform === 'win32' && (/64/.test(process.arch) || process.env.PROCESSOR_ARCHITEW6432)) {
        return 'win64';
      }
  ...
  var userAgent = (0, _get2.default)(window, 'navigator.userAgent', '');
  if (/windows/i.test(userAgent)) {
    if (/(x86.|win|amd|wow|x64_)64/i.test(userAgent) || /x64;/i.test(userAgent)) {
      return 'win64';
    }
    return 'win32';
